...
async def mention_afk(mention):
    ...
    if ISAFK and mention.message.mentioned:
        ...
        if mention.sender_id not in USERS or chat_title not in USERS:
            if AFKREASON:
                await mention.reply(
                    f"**GUA OFF DULU YA NJENG.** (Since: {afk_str})"
                    f"\nReason: `{AFKREASON}`."
                )
            else:
                await mention.reply(
            ...
            if USERS[mention.sender_id] % randint(2, 4) == 0:
                if AFKREASON:
                    await mention.reply(
                        f"**GUA LAGI OFF ASUH.** (Since: {afk_str})"
                        f"\nReason: `{AFKREASON}`."
                    )
                else:
                    await mention.reply(
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit("**BYE KNTOL LU SEMUA!**" f"\nReason: `{string}`")
    else:
        await afk_e.edit("**BYE KNTOL LU SEMUA!**")
    if BOTLOG:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.edit("**GUA UDH ON NIH KNTOL!**")
